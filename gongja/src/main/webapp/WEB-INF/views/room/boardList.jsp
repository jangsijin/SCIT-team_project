...
<table class="table">
    ...
    <c:forEach var="i" items="${board}">
      <tr>
        <c:if test="${i.code == 0 }"><td><span style="color:black;">[자유]</span><a href="board?num=${i.num}&room_num=${i.room_num}">${i.title}</a></td></c:if>
        <c:if test="${i.code == 1 }"><td><strong>[공지]<a href="board?num=${i.num}&room_num=${i.room_num}">${i.title}</a></strong></td></c:if>
        <td>${i.id}</td>
        <td>${i.inputdate}</td>
      </tr>
    </c:forEach>
